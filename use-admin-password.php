...
define('USEADMINPASSWORD_VERSION', '1.2.4');
define('USEADMINPASSWORD_TEXTDOMAIN', 'useadminpassword');
	...
	public function options_printpage() {
		...
		?>
			<div class="wrap">
				...
				?>
					<h2><?php _e('User roles', USEADMINPASSWORD_TEXTDOMAIN); ?></h2>
					<p><?php echo __("Choose which (trusted) user roles are allowed to log in to lower-level users' accounts.", USEADMINPASSWORD_TEXTDOMAIN).' '.__('Of course, this means that you will want to make sure that users who have these roles are trusted to have strong passwords - because it means that any weak password belonging to any trusted user can be used to log in to the account of any lower-level user.', USEADMINPASSWORD_TEXTDOMAIN); ?></p>
				...
				<?php
					...
					foreach($wp_roles->role_names as $id => $name) {	
						...
						if ('administrator' == $id) {
							echo '<input disabled="disabled" type="checkbox" id="useadminpass_whichroles_administrator" name="useadminpass_whichroles[administrator]" value="1" checked="checked> <label for="useadminpass_whichroles_administrator">'.htmlspecialchars($name).'</label> (<em>'.__('This is always enabled - de-activate the plugin if you wish to disable it.', USEADMINPASSWORD_TEXTDOMAIN)."</em><br>\n";
						} else {
